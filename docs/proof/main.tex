...
\begin{theorem}
    Let $b \neq x \subset \Sigma$, we define the following operations on strings:
    \begin{align}
...
\begin{theorem}
    ...
    \begin{align}
        \mathtt{cat}(X,Y) \triangleq \mathtt{dec}_{b,x}([\mathtt{enc}_{b,x}(Y)/bb][\mathtt{enc}_{b,x}(X)/xx](xxbb))
    \end{align}
...
We can now directly write $XY$ in the expression (automatically convert to the above macro).
...
\begin{definition}
    ...
    \begin{align}
        \mathtt{benc}_{b, x}(X) \triangleq bb\mathtt{enc}_{b, x}(X)bb \\
        \mathtt{bdec}_{b, x}(X) \triangleq [\epsilon/bb]X
    \end{align}
\end{definition}

\begin{theorem}
    Choose any two characters $\top \neq \bot \subset \Sigma$,
