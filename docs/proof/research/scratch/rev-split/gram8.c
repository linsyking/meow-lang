/* rev-split ROUND 8 -- HARDENING: periodic index masks + two-color
 * profiles (charter: coordinator round-8 message).  Family D(k;3).
 *
 * Round 7's machine gap, closed: the [gram] engine implemented INTERVAL
 * index sets only; the formalism's Ind predicates include residues.
 * Round 8 adds:
 *   (1) PERIODIC MASKS on index sets: j in [la*k+lb .. ha*k+hb] iff
 *       j mod mmod == mres (mmod = 0: no mask).  The masked family is
 *       still ONE directive -- k-independent grammar size by
 *       construction.
 *   (2) PER-CELL GRAMMARS selected by k mod 2 (the formalism's
 *       per-cell structure, made executable).
 *   (3) TWO-COLOR RUN PROFILES: the profile records BOTH a-runs and
 *       b-runs: [a_0, b_0, a_1, b_1, ..., a_m], b_i >= 1, interior
 *       a_i >= 1, end a's >= 0.  WHY: outputs with CONSECUTIVE b's
 *       (zero a-runs in the round-7 view) have exponentially many
 *       zero-runs -- an a-run-only profile would break the polynomial
 *       expansion reconciliation.  The two-color profile absorbs the
 *       zero-structure into b-run VALUES (anchored exponential forms),
 *       keeping the expansion polynomial.  Round 7's 17 grammars are
 *       b-simple (all b-runs = 1 -- SELF-CHECKED below), which is
 *       exactly why the a-run-only view worked there.
 *
 * The two new expressions (hand-derived, then machine-confirmed):
 *   tile4 = [b/'aaaa']X:  run j tiles from offset 0: floor(3^j/4)
 *       windows -> 'b', remnant Lambda_j = 3^j mod 4 = 1,3,1,3,...
 *       PERIODIC in j with period 2.  Two-color profile:
 *       OF j=0..k-1: [a=Lambda_j; b=1+floor(3^{j+1}/4)]; a=Lambda_k.
 *   mod2  = [b/'bab']tile4:  the multi-b pattern 'bab' (c_0=0, c_1=1,
 *       c_2=0) fires exactly at the b-pairs (last window-b of run j,
 *       separator j) whose interior is Lambda_j: Lambda_j = 1 iff
 *       3^j = 1 mod 4 iff j EVEN.  FIRED SET = {even j in [2..k-1]}:
 *       a residue class mod 2 with a V-fixed pre-period -- GENUINELY
 *       PERIODIC, not an interval, not a threshold.  The grammar's
 *       surviving family is the ODD residue class:
 *       k odd:  a=1; b=1; a=3; OF odd j=3..k: [b=merged; a=3]
 *       k even: a=1; b=1; a=3; OF odd j=3..k-1: [b=merged; a=3];
 *               b=1+floor(3^k/4) (untouched); a=Lambda_k = 1
 *       where merged(j) = floor(3^{j-1}/4)+1+floor(3^j/4).
 *
 * [meas] mode (candidate (b), the INV3 measure form on the same
 * executable footing): for tile4, mod2, decb, thresh the hand EPT
 * closed forms of the a-count and b-count (geometric sums over
 * residue classes) are checked THREE WAYS: against the text counts,
 * against the grammar-expansion sums, per cell.  This is the channel
 * Lane D's OL-2 wants -- the FORM, not the proofs.
 *
...
static void expand2(const Drec*d,int nd,int k,int amb){
    for(int i=0;i<nd;i++){
        ...
        if(D->kind==0) emitv(FORM[D->fid](k,amb));
            ...
            for(int j=lo;j<=hi;j++){
                if(D->mmod && (j%D->mmod)!=D->mres) continue;
                emitv(FORM[D->fid](k,j));
            }
        } else {
            int lo=D->la*k+D->lb, hi=D->ha*k+D->hb;
            for(int c=lo;c<=hi;c++){
                if(D->mmod && (c%D->mmod)!=D->mres) continue;
...
static void mode_gram(int kforce){
    printf("=== [gram] ROUND 8: two-color profiles + periodic index masks ===\n");
