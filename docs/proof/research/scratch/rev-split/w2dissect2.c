...
static void suppS(int e,int S){
    int va[2048],vb[2048],ka=0,kb=0,nd=0;
    for(int i=1;i<S-1;i++)for(int j=1;j<S-i;j++){
        ...
        if(!d&&ka<2048) va[ka++]=a;
        d=0; for(int t=0;t<kb;t++) if(vb[t]==b){d=1;break;}
        if(!d&&kb<2048) vb[kb++]=b;
    }
    printf("   S=%2d: a-support %3d b-support %3d (nd=%d)\n",S,ka,kb,nd);
}
...
int main(int argc,char**argv){
    ...
    for(long t=0;t<=target;t++){
        ...
        if(t==target){
            printf("=== idx %ld: ",target); ppn(e); printf("\n");
            for(int S=9;S<=33;S+=3) suppS(e,S);
        }
    }
    return 0;
}
/* path test appended via main2 in separate build below */
